...
SVR_ANSWER_PROB = math.sqrt(1)  # up*down prob.
...
def decode_status(data):
    ...
    for bit_pos in range(2, 8):
        if (indications & (1 << bit_pos)) > 0:
            payload[keys[bit_pos - 2]] = struct.unpack(
                ">H", data[sensor * 2 + 1 : sensor * 2 + 3]
            )[0]
            sensor = sensor + 1
...
def encode_status_ack(cmd):
    ...
    if cmd["alarm_a"]:
        indication = indication | (1 << 0)
    if cmd["alarm_b"]:
        indication = indication | (1 << 1)
    if cmd["accel_x"]:
        indication = indication | (1 << 2)
    if cmd["accel_y"]:
        indication = indication | (1 << 3)
    if cmd["accel_z"]:
        indication = indication | (1 << 4)
    if cmd["giros_x"]:
        indication = indication | (1 << 5)
    if cmd["giros_y"]:
        indication = indication | (1 << 6)
    if cmd["giros_z"]:
        indication = indication | (1 << 7)
...
def server():
    ...
    # data_sub.connect("tcp://192.168.5.140:60000")
    # data_sub.setsockopt_string(zmq.SUBSCRIBE,"")

    # data_pub = context.socket(zmq.PUB)
    # data_pub.bind("tcp://192.168.5.140:60001")
